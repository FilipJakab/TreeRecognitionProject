...
class DemoController(Resource):
	def post(self):
		...
		if not self.check_file_extension(image_filename):
			print("image doenst contain valid file extension")
			response_obj["message"] = "image doenst contain valid file extension"
			return response_obj
	...
	def check_file_extension(self, filename):
		return filename[(filename.index(".") + 1):] in app.config["ALLOWED_IMAGE_EXETENSION"]

# example of method return
# return { "message": "200 OK" }, 200, { "custom-header": time.gmtime() }

api.add_resource(DemoController, "/")
